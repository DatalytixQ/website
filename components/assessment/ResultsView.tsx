...
import { CheckCircle, AlertTriangle, Calendar, Download } from "lucide-react";
  ...
  return (
    <div className="w-full max-w-4xl mx-auto flex flex-col space-y-8 pb-12">
      ...
      >
        ...
        <p className={`text-lg font-medium ${levelColor}`}>Nivel: {friction_level}</p>
      ...
      {findings && findings.length > 0 && (
        ...
        >
          ...
          <div className="mt-8 pt-4 border-t border-border/50">
            <p className="text-xs italic text-muted-foreground opacity-80">Estos hallazgos son preliminares y están diseñados para orientar una conversación estratégica.</p>
          </div>
        </motion.div>
      )}
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
          ...
          <button
            disabled
            className="w-full sm:w-auto border border-border text-muted-foreground font-medium px-8 py-4 rounded-md inline-flex items-center justify-center gap-2 cursor-not-allowed opacity-60 transition-all"
          >
            <Download size={18} />
            Descargar Reporte PDF (Fase 4)
          </button>
        </div>
      </motion.div>
    </div>
